...
// Deployed Cloudflare proxy URL
const PROXY_URL = "https://atmopulse.ayushkunkulol5.workers.dev";
...
function useAuth() {
  ...
  const signup = (email: string, pass: string, name: string) => {
    ...
    const u = {
      email: e, name: name?.trim() || e.split("@")[0], provider: "email",
  ...
  const suggest = useCallback(async (v: string) => {
    ...
    try {
      const [nomRes, waqiRes] = await Promise.allSettled([
        ...
        fetch(`${PROXY_URL}/search/?keyword=${encodeURIComponent(v)}`).then(r => r.json()),
      ]);
...
export default function App() {
  ...
  const fetchWAQI = useCallback(async (lat: number | null, lon: number | null, waqiUid: any, cityName: string | null) => {
    const tryUrl = async (url: string) => {
      ...
      if (j.status === "ok" && j.data && typeof j.data !== "string") return parseWAQI(j);
      return null;
    };

    // Try in priority order: UID → geo coords → city name
    if (waqiUid) {
      const result = await tryUrl(`${PROXY_URL}/feed/@${waqiUid}/`);
      if (result) return result;
    }
    if (lat && lon) {
      const result = await tryUrl(`${PROXY_URL}/feed/geo:${lat};${lon}/`);
      if (result) return result;
    }
    if (cityName) {
      const result = await tryUrl(`${PROXY_URL}/feed/${encodeURIComponent(cityName)}/`);
      if (result) return result;
    }
    throw new Error("No air quality data found for this location. Please try a different city.");
  }, []);
  ...
  const doLoad = useCallback(async (lat: number | null, lon: number | null, waqiUid: any, cityName: string | null) => {
    ...
    try {
      ...
      let fCity: string;
      if (cityName) {
        // User searched for a city by name — use what they typed / selected
        fCity = cityName;
      } else if (origLat && origLon) {
        // User used GPS detect — reverse-geocode their real coordinates
        const rgName = await reverseGeocode(origLat, origLon);
        fCity = rgName || unified?.city || "Unknown";
      } else {
        // Fallback to the WAQI station name
        fCity = unified?.city || "Unknown";
      }
